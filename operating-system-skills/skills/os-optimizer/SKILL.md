...
# OS Optimizer — Vault Audit

Run 9 frameworks across every markdown file in the vault. For each one, read its pass-implementation file, run the checks, log what it finds, walk the user through the fixes one at a time, and either apply them or save them to a plan. Finish with a single HTML report grouped by framework. **Never paste the HTML into chat — return only the saved path and a one-paragraph summary.**
...
Frame this as "here's the structure I see in your vault" — not "here's what's missing from a checklist." The user's structure is the source of truth; the Brody AI standard taxonomy is just one of the lenses the agent uses to recognize patterns.
...
I'll audit this structure as it stands. F9.0 may suggest specific structural improvements where I see concrete evidence they'd help your vault (not just because Brody AI uses them) — you'll review each suggestion in walk. Confirmed assignments persist to `.claude/vault-roles.json`.
...
- **Never** propose a structural change purely because Brody AI uses that convention. F9.0 tier-2 findings (functional improvements suggesting Brody AI conventions) must cite specific evidence in *this* vault that the convention would resolve a real problem. F9.6 reorg proposals must cite the user's own stated folder purposes. "Brody AI does X" is not a valid justification anywhere in the optimizer. The Brody AI taxonomy is a recognition lens and an optional reference — never a target shape the user must conform to.
