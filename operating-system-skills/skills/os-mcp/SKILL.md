...
# OS MCP — Reach Your Vault From Anywhere

This skill deploys the Relay MCP server to the user's own Railway account so Claude can reach their vault from any device — not just when their laptop is open. The MCP source is bundled at `${CLAUDE_PLUGIN_ROOT}/skills/os-mcp/reference/relay-mcp-server/` — copy from there, never fetch it elsewhere. Once live, any Claude client (Claude Code or claude.ai) can read and write the vault over the Relay.md sync protocol, which is what lets the Operator and Optimizer run autonomously as cloud routines.
